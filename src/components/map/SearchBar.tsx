...
const SearchBar = ({ isRoutingMode, selectingPoint, onSelectResult }: SearchBarProps) => {
  ...
  return (
    <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 z-30 w-full max-w-xs sm:max-w-md px-2 sm:px-4">
      <div className="relative">
        <div className="relative flex items-center">
          <Search className="absolute left-3 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 pointer-events-none" />
          <input
            ref={searchInputRef}
            type="text"
            value={searchQuery}
            ...
            placeholder={isRoutingMode && selectingPoint 
              ...
              : "Search places in Islamabad..."
            }
            className="w-full pl-9 sm:pl-10 pr-9 sm:pr-10 py-2.5 sm:py-3 bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary placeholder:text-gray-400 touch-manipulation"
          />
          {isSearching && (
            <Loader2 className="absolute right-3 w-4 h-4 sm:w-5 sm:h-5 text-primary animate-spin" />
          )}
          {!isSearching && searchQuery && (
            <button
              ...
              className="absolute right-3 p-0.5 hover:bg-gray-100 rounded touch-manipulation"
            >
        ...
        <AnimatePresence>
          ...
          {showSearchResults && !isSearching && searchResults.length > 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-200 overflow-hidden max-h-60 sm:max-h-80 overflow-y-auto"
            >
              ...
              {searchResults.map((result) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 hover:bg-gray-50 transition-colors text-left border-b border-gray-100 last:border-b-0 touch-manipulation"
                  >
                    <div className="p-1.5 sm:p-2 bg-primary/10 rounded-lg flex-shrink-0">
                      <PlaceIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
                    </div>
                    ...
                    {isRoutingMode && selectingPoint && (
                      <div className={`px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-medium rounded ${
                        selectingPoint === 'start' 
                          ? 'bg-emerald-100 text-emerald-700' 
                          : 'bg-red-100 text-red-700'
                      }`}>
                        {selectingPoint === 'start' ? 'A' : 'B'}
                      </div>
                    )}
                  </button>
                );
